...
async def test_execution_time():
    ...
    # The uncached and first cached calls both execute the slow path.
    assert time_without_cache >= 1.5
    assert time_with_cache_first >= 1.5

    # The second cached call should return from Redis without the slow path.
    assert time_with_cache_second < time_with_cache_first
    assert time_with_cache_second < 0.1
